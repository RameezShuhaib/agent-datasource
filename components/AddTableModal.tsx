...
export const AddTableModal: React.FC<AddTableModalProps> = ({ onClose, onSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-slate-900 rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh] border border-slate-800">
        ...
        <div className="flex border-b border-slate-800 px-6">
          <button 
            ...
            className={`py-3 px-4 text-sm font-bold border-b-2 transition-colors ${activeTab === 'manual' ? 'border-blue-500 text-blue-400' : 'border-transparent text-slate-400 hover:text-slate-200'}`}
          >
            Manual Schema
          </button>
          <button 
            onClick={() => setActiveTab('csv')}
            className={`py-3 px-4 text-sm font-bold border-b-2 transition-colors ${activeTab === 'csv' ? 'border-blue-500 text-blue-400' : 'border-transparent text-slate-400 hover:text-slate-200'}`}
          >
            Import CSV
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
          <div className="space-y-1">
            ...
            <input 
              type="text" 
              className="w-full px-4 py-2 border border-slate-700 bg-slate-950 rounded-lg text-sm text-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none placeholder-slate-600"
          ...
          {activeTab === 'manual' ? (
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 px-3 py-2 bg-slate-950 rounded-lg border border-slate-800">
                  <div className="flex-1 text-xs font-bold text-slate-500 uppercase">id</div>
                  <div className="w-32 text-xs font-bold text-slate-500 uppercase italic">Primary Key</div>
                ...
                {columns.map((col, idx) => (
                  <div key={idx} className="flex items-center gap-3 group">
                    <input 
                      type="text" 
                      className="flex-1 px-3 py-2 border border-slate-700 bg-slate-950 rounded-lg text-sm text-white outline-none focus:border-blue-500"
                      placeholder="Column name"
                      value={col.name}
                      onChange={(e) => updateColumn(idx, 'name', e.target.value)}
                    />
                    <select 
                      className="w-32 px-3 py-2 border border-slate-700 bg-slate-950 rounded-lg text-sm text-white outline-none"
                      value={col.type}
            ...
            <div className="space-y-4">
              {!csvData ? (
                <div 
                  ...
                  className="border-2 border-dashed border-slate-700 bg-slate-950 rounded-xl p-12 text-center hover:border-blue-500 hover:bg-slate-900 transition-all cursor-pointer group"
                >
                  ...
                  <div className="bg-slate-800 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:bg-blue-900/30 transition-colors">
                    <svg className="w-8 h-8 text-slate-500 group-hover:text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                    </svg>
                  </div>
                  <p className="text-slate-200 font-bold">Upload CSV to infer schema</p>
                  <p className="text-slate-500 text-sm mt-1">First row will be used as column names</p>
                </div>
              ) : (
                <div className="space-y-4">
                  <div className="bg-blue-900/20 p-4 rounded-xl border border-blue-900/30 flex items-center gap-3">
                    <svg className="w-5 h-5 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <div className="flex-1">
                      <p className="text-sm font-bold text-blue-300">CSV Loaded Successfully</p>
                      <p className="text-xs text-blue-400">{csvData.rows.length} records found with {csvData.headers.length} columns.</p>
                    </div>
                    <button onClick={() => setCsvData(null)} className="text-xs font-bold text-blue-400 hover:underline">Reset</button>
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Inferred Columns</label>
                    <div className="flex flex-wrap gap-2">
                      {csvData.headers.map(h => (
                        <span key={h} className="px-2 py-1 bg-slate-800 border border-slate-700 rounded text-xs font-medium text-slate-300">{h}</span>
                      ))}
                    </div>
                  </div>
                </div>
